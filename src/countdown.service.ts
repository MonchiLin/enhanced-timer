...
const DEFAULT_CONFIG: ConsConfig = {
  precision: 100,
  mode: "RAF",
  log: false,
  token: "default"
...
class CountdownService {
  ...
  private countdownConfig: CountdownConfig | null = null;
  ...
  private rectifyTime() {
    ...
    this.config.log && console.log("期望的当前剩余时间 =>", timeOfAnticipation / 1000, "s");
    this.config.log && console.log("实际当前剩余时间 =>", this.currentTime, "s");
    // 偏差 = 当前的倒计时 - 期望的当前剩余时间 / 1000 （因为期望的剩余时间是时间戳）
    const offset = this.currentTime - timeOfAnticipation / 1000;
    this.config.log && console.log("误差 =>", offset, "s");
